...
public class ExternalCalendarViewModel extends SelectorComposer<Component> {

	private static final long serialVersionUID = 1L;

	@Wire
	private Calendars externalCalendars;
	@Wire
	...
	public void doAfterCompose(Component comp) throws Exception {
		...
		externalCalendars.setModel(this.calendarModel);
	}

	// control the calendar position
	@Listen("onClick = #today")
	public void gotoToday() {
		TimeZone timeZone = externalCalendars.getDefaultTimeZone();
		externalCalendars.setCurrentDate(Calendar.getInstance(timeZone).getTime());
	}

	@Listen("onClick = #next")
	public void gotoNext() {
		externalCalendars.nextPage();
	}

	@Listen("onClick = #prev")
	public void gotoPrev() {
		externalCalendars.previousPage();
	}

	// control page display
	@Listen("onClick = #pageDay")
	public void changeToDay() {
		externalCalendars.setMold("default");
		externalCalendars.setDays(1);
	}

	@Listen("onClick = #pageWeek")
	public void changeToWeek() {
		externalCalendars.setMold("default");
		externalCalendars.setDays(7);
	}

	@Listen("onClick = #pageMonth")
	public void changeToYear() {
		externalCalendars.setMold("month");
	}
	...
	public void applyFilter() {
		...
		externalCalendars.setModel(calendarModel);
	}
