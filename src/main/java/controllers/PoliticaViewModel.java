...
public class PoliticaViewModel {
	...
	public void eliminar(@BindingParam("Politica") final Politica politica) {
		...
		Messagebox.show("Estas seguro de eliminar " + politica.getDescripcion(), "Confirmar",
					...
					public void onEvent(Event evt) throws InterruptedException {
						if (evt.getName().equals("onOK")) {
							try {
								...
								reglasAll = politicaDao.obtenerTodos();
								Messagebox.show("La politica ha sido eliminada", "Exito", Messagebox.OK, Messagebox.INFORMATION);
								BindUtils.postGlobalCommand(null, null, "refreshReglas", null);
							} catch (Exception e) {
								Messagebox.show(e.getMessage(), "La politica no se pudo eliminar", Messagebox.OK, Messagebox.ERROR);
							}
						}
					}
				});
