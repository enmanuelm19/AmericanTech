...
public class TipoActividadViewModel {
	...
	private List<TipoActividad> tiposAll;
	...
	public void init() throws Exception {
		...
		tiposAll = new ArrayList<TipoActividad>();
		tipoDao = new TipoActividadDao();
		tiposAll = tipoDao.obtenerTodos();
	}

	public ListModelList<TipoActividad> getAllTipoActividad() {

		return new ListModelList<TipoActividad>(tiposAll);
	}
	...
	@Command
	public void showModal(@BindingParam("Tipo") TipoActividad tipo) {
		Map<String, Object> args = new HashMap<String, Object>();
		args.put("TipoActividad", tipo);
		Window window = (Window) Executions.createComponents("configuracion/categoria/registrarTipoActividad.zul",
				null, args);
		window.doModal();
	}

	@Command
	@NotifyChange({ "allTipoActividad", "cantRegistros" })
	public void eliminar(@BindingParam("Tipo") final TipoActividad tipo) {
				...
				Messagebox.OK | Messagebox.CANCEL, Messagebox.QUESTION, new org.zkoss.zk.ui.event.EventListener() {
					public void onEvent(Event evt) throws InterruptedException {
						if (evt.getName().equals("onOK")) {
							try {
								tipoDao.eliminarTipoActividad(tipo);
								tiposAll = tipoDao.obtenerTodos();
	...
	public void filtro() throws Exception {
		List<TipoActividad> tip = new ArrayList<TipoActividad>();
		String desc = getDescFiltro().toLowerCase();
		String id = getIdFiltro().toLowerCase();

		for (Iterator<TipoActividad> i = tipoDao.obtenerTodos().iterator(); i.hasNext();) {
			TipoActividad tmp = i.next();
			if (tmp.getDescripcion().toLowerCase().contains(desc)
					&& String.valueOf(tmp.getIdTipoActividad()).toLowerCase().contains(id)) {
				tip.add(tmp);
			}
		}
		tiposAll = tip;
	...
	@GlobalCommand
	@NotifyChange({ "allTipoActividad", "cantRegistros" })
	public void refreshTipoActividad() throws Exception {
		tiposAll = tipoDao.obtenerTodos();
	}
}
